...
class TransformerRunner:
    ...
    def train_for_epoch(
        ...
        for source, source_lens, targets in dataloader:
            ...
            # Split the targets
            model_inputs, criterion_targets = self.train_input_target_split(targets)

            # Get the logits
            # DO I NORMALIZE LOGITS OR NOT? (ITS A VARIABLE IN THE FORWARD FUNCTION FOR THE ENCODERDECODER)
            logits = self.model(source, model_inputs, True).to(device)

            # Get the loss
            #NEED TO PASS INPUTS TO CRITERION
            ...
            if (i + 1) % accum_iter == 0 or (i+1) == len(dataloader): 
                ...
                if i in {19, 199, 999}:
                    ...
                    print(f"Forward Step: {i + 1}/{len(dataloader)} | "
                      f"Accumulation Step: {i // accum_iter} | "
                      f"Loss: {loss.item() * accum_iter:.2f} | "
                      f"Learning Rate: {current_lr:.2e}", flush=True)
    ...
    def translate(self, input_sentence):
        ...
        # === ============== === #
        tokenized_words = self.dataset.tokenize(input_sentence)

        source_tokens = [ self.dataset.source_word2id.get(token, 
                    self.dataset.source_unk_id) for token in tokenized_words]
        ...
        # === ============== === #
        output = []
        for word in hypotheses.squeeze(0):
            output.append(self.dataset.target_id2word.get(word.item(), '<unk>'))
    ...
    def compute_batch_total_bleu(
        ...
        """
        bleu_score = [0.0] * len(n_gram_levels)
        for i in range(target_y_ref.size(0)):
            reference = target_y_ref[i].tolist()
            reference_removed = [ref for ref in reference if (ref != sos_idx and 
                                    ref != eos_idx and ref != pad_idx)]
            candidate = target_y_cand[i].tolist()
            candidate_removed = [cand for cand in candidate if (cand != sos_idx and 
                                    cand != eos_idx and cand != pad_idx)]
            for j in range(len(n_gram_levels)):
                bleu_score[j] += bleu_score_func(reference_removed, 
                                            candidate_removed, n_gram_levels[j])
        return tuple(bleu_score), target_y_ref.size(0)
    #NEED TO CHECK
